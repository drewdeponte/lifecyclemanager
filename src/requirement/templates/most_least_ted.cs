...
<?cs def:selected( opt, tt ) ?>
    <?cs if: tt == opt ?>
        selected
    <?cs /if ?>
<?cs /def ?>

<div style="position: absolute; right: 0;" id="ted_dropdown">
    <form name="tt" action="" method="">
        <select onchange="window.location='<?cs var: view_ted ?>' + document.forms['tt'].ted.value" name="ted">
            <option <?cs call:selected( "Tasks", ticket_type ) ?> name="Tasks">Tasks</option>
            <option <?cs call:selected( "Enhancements", ticket_type ) ?> name="Enhancements">Enhancements</option>
            <option <?cs call:selected( "Defects", ticket_type ) ?> name="Defects">Defects</option>
        </select>
    </form>
</div>

<h1>Most To Least Cited Requirements Associated To: <?cs var: ticket_type ?></h1>
    <div id="description">
        <p>Show the most to least cited <?cs var: ticket_type ?></p>
    </div>
<?cs if: noresults ?>
